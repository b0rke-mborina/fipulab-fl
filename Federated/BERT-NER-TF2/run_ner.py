...
def main():
    ...
    if args.do_train:
        ...
        loss_fct = tf.keras.losses.SparseCategoricalCrossentropy(reduction=tf.keras.losses.Reduction.NONE)
        loss11_fct = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True, reduction=tf.keras.losses.Reduction.NONE)
    ...
    if args.do_eval:
        ...
        if args.eval_on == "test":
            output_eval_file = os.path.join(args.output_dir, "eval_results_on_test.txt")
            with open(output_eval_file, "w") as writer:
                logger.info("***** Eval results *****")
                logger.info("\n%s", report)
                writer.write(report)
        else:
            output_eval_file = os.path.join(args.output_dir, "eval_results_on_valid.txt")
            with open(output_eval_file, "w") as writer:
                logger.info("***** Eval results *****")
                logger.info("\n%s", report)
                writer.write(report)
